/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * IoT Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>DeviceVendorCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity DeviceVendor.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>IoT Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core IoT device, connectivity, telemetry, twin, command, security, and operations capabilities.</td></tr>
 *          <tr><td>industry</td><td>Internet of Things</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	DeviceVendorService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateDeviceVendorCommand</li>
 *      <li>UpdateDeviceVendorCommand</li>
 *      <liDeleteDeviceVendorCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/DeviceVendorCommand")
public class DeviceVendorCommandRestController extends BaseSpringRestController {

	public DeviceVendorCommandRestController( DeviceVendorService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a DeviceVendor.
     * @param		DeviceVendor	deviceVendor
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateDeviceVendorCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createDeviceVendor( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a DeviceVendor.  if no key provided, calls create, otherwise calls save
     * @param		DeviceVendor deviceVendor
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateDeviceVendorCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateDeviceVendorCommand
			// -----------------------------------------------
			completableFuture = service.updateDeviceVendor(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "DeviceVendorController:update() - successfully update DeviceVendor - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a DeviceVendor entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID deviceVendorId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteDeviceVendorCommand command = new DeleteDeviceVendorCommand( deviceVendorId );

    	try {
        	DeviceVendorService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted DeviceVendor with key " + command.getDeviceVendorId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	


    /**
     * save DeviceModels on DeviceVendor
     * @param		command AssignDeviceModelsToDeviceVendorCommand
     */     
	@PutMapping("/addToDeviceModels")
	public void addToDeviceModels( @RequestBody(required=true) AssignDeviceModelsToDeviceVendorCommand command ) {
		try {
			service.addToDeviceModels( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set DeviceModels", exc );
		}
	}

    /**
     * remove DeviceModels on DeviceVendor
     * @param		command RemoveDeviceModelsFromDeviceVendorCommand
     */     	
	@PutMapping("/removeFromDeviceModels")
	public void removeFromDeviceModels( 	@RequestBody(required=true) RemoveDeviceModelsFromDeviceVendorCommand command )
	{		
		try {
			service.removeFromDeviceModels( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set DeviceModels", exc );
		}
	}

    /**
     * save FirmwareReleases on DeviceVendor
     * @param		command AssignFirmwareReleasesToDeviceVendorCommand
     */     
	@PutMapping("/addToFirmwareReleases")
	public void addToFirmwareReleases( @RequestBody(required=true) AssignFirmwareReleasesToDeviceVendorCommand command ) {
		try {
			service.addToFirmwareReleases( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set FirmwareReleases", exc );
		}
	}

    /**
     * remove FirmwareReleases on DeviceVendor
     * @param		command RemoveFirmwareReleasesFromDeviceVendorCommand
     */     	
	@PutMapping("/removeFromFirmwareReleases")
	public void removeFromFirmwareReleases( 	@RequestBody(required=true) RemoveFirmwareReleasesFromDeviceVendorCommand command )
	{		
		try {
			service.removeFromFirmwareReleases( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set FirmwareReleases", exc );
		}
	}

    /**
     * save HardwareModules on DeviceVendor
     * @param		command AssignHardwareModulesToDeviceVendorCommand
     */     
	@PutMapping("/addToHardwareModules")
	public void addToHardwareModules( @RequestBody(required=true) AssignHardwareModulesToDeviceVendorCommand command ) {
		try {
			service.addToHardwareModules( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set HardwareModules", exc );
		}
	}

    /**
     * remove HardwareModules on DeviceVendor
     * @param		command RemoveHardwareModulesFromDeviceVendorCommand
     */     	
	@PutMapping("/removeFromHardwareModules")
	public void removeFromHardwareModules( 	@RequestBody(required=true) RemoveHardwareModulesFromDeviceVendorCommand command )
	{		
		try {
			service.removeFromHardwareModules( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set HardwareModules", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected DeviceVendor deviceVendor = null;
	protected DeviceVendorService service = null;
    private static final Logger LOGGER = Logger.getLogger(DeviceVendorCommandRestController.class.getName());
    
}
