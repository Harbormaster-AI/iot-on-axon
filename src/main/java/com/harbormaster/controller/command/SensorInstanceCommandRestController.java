/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * IoT Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>SensorInstanceCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity SensorInstance.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>IoT Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core IoT device, connectivity, telemetry, twin, command, security, and operations capabilities.</td></tr>
 *          <tr><td>industry</td><td>Internet of Things</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	SensorInstanceService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateSensorInstanceCommand</li>
 *      <li>UpdateSensorInstanceCommand</li>
 *      <liDeleteSensorInstanceCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/SensorInstanceCommand")
public class SensorInstanceCommandRestController extends BaseSpringRestController {

	public SensorInstanceCommandRestController( SensorInstanceService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a SensorInstance.
     * @param		SensorInstance	sensorInstance
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateSensorInstanceCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createSensorInstance( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a SensorInstance.  if no key provided, calls create, otherwise calls save
     * @param		SensorInstance sensorInstance
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateSensorInstanceCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateSensorInstanceCommand
			// -----------------------------------------------
			completableFuture = service.updateSensorInstance(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "SensorInstanceController:update() - successfully update SensorInstance - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a SensorInstance entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID sensorInstanceId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteSensorInstanceCommand command = new DeleteSensorInstanceCommand( sensorInstanceId );

    	try {
        	SensorInstanceService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted SensorInstance with key " + command.getSensorInstanceId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Device on SensorInstance
     * @param		command AssignDeviceToSensorInstanceCommand
     */     
	@PutMapping("/assignDevice")
	public void assignDevice( @RequestBody AssignDeviceToSensorInstanceCommand command ) {
		try {
			service.assignDevice( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Device", exc );
        }
	}

    /**
     * unassign Device on SensorInstance
     * @param		 command UnAssignDeviceFromSensorInstanceCommand
     */     
	@PutMapping("/unAssignDevice")
	public void unAssignDevice( @RequestBody(required=true)  UnAssignDeviceFromSensorInstanceCommand command ) {
		try {
			service.unAssignDevice( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Device", exc );
		}
	}
	

    /**
     * save TelemetryStreams on SensorInstance
     * @param		command AssignTelemetryStreamsToSensorInstanceCommand
     */     
	@PutMapping("/addToTelemetryStreams")
	public void addToTelemetryStreams( @RequestBody(required=true) AssignTelemetryStreamsToSensorInstanceCommand command ) {
		try {
			service.addToTelemetryStreams( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set TelemetryStreams", exc );
		}
	}

    /**
     * remove TelemetryStreams on SensorInstance
     * @param		command RemoveTelemetryStreamsFromSensorInstanceCommand
     */     	
	@PutMapping("/removeFromTelemetryStreams")
	public void removeFromTelemetryStreams( 	@RequestBody(required=true) RemoveTelemetryStreamsFromSensorInstanceCommand command )
	{		
		try {
			service.removeFromTelemetryStreams( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set TelemetryStreams", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected SensorInstance sensorInstance = null;
	protected SensorInstanceService service = null;
    private static final Logger LOGGER = Logger.getLogger(SensorInstanceCommandRestController.class.getName());
    
}
