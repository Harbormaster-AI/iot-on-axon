/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * IoT Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>UsageRecordCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity UsageRecord.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>IoT Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core IoT device, connectivity, telemetry, twin, command, security, and operations capabilities.</td></tr>
 *          <tr><td>industry</td><td>Internet of Things</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	UsageRecordService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateUsageRecordCommand</li>
 *      <li>UpdateUsageRecordCommand</li>
 *      <liDeleteUsageRecordCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/UsageRecordCommand")
public class UsageRecordCommandRestController extends BaseSpringRestController {

	public UsageRecordCommandRestController( UsageRecordService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a UsageRecord.
     * @param		UsageRecord	usageRecord
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateUsageRecordCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createUsageRecord( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a UsageRecord.  if no key provided, calls create, otherwise calls save
     * @param		UsageRecord usageRecord
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateUsageRecordCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateUsageRecordCommand
			// -----------------------------------------------
			completableFuture = service.updateUsageRecord(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "UsageRecordController:update() - successfully update UsageRecord - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a UsageRecord entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID usageRecordId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteUsageRecordCommand command = new DeleteUsageRecordCommand( usageRecordId );

    	try {
        	UsageRecordService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted UsageRecord with key " + command.getUsageRecordId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Tenant on UsageRecord
     * @param		command AssignTenantToUsageRecordCommand
     */     
	@PutMapping("/assignTenant")
	public void assignTenant( @RequestBody AssignTenantToUsageRecordCommand command ) {
		try {
			service.assignTenant( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Tenant", exc );
        }
	}

    /**
     * unassign Tenant on UsageRecord
     * @param		 command UnAssignTenantFromUsageRecordCommand
     */     
	@PutMapping("/unAssignTenant")
	public void unAssignTenant( @RequestBody(required=true)  UnAssignTenantFromUsageRecordCommand command ) {
		try {
			service.unAssignTenant( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Tenant", exc );
		}
	}
	
    /**
     * save Device on UsageRecord
     * @param		command AssignDeviceToUsageRecordCommand
     */     
	@PutMapping("/assignDevice")
	public void assignDevice( @RequestBody AssignDeviceToUsageRecordCommand command ) {
		try {
			service.assignDevice( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Device", exc );
        }
	}

    /**
     * unassign Device on UsageRecord
     * @param		 command UnAssignDeviceFromUsageRecordCommand
     */     
	@PutMapping("/unAssignDevice")
	public void unAssignDevice( @RequestBody(required=true)  UnAssignDeviceFromUsageRecordCommand command ) {
		try {
			service.unAssignDevice( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Device", exc );
		}
	}
	
    /**
     * save ConnectivityPlan on UsageRecord
     * @param		command AssignConnectivityPlanToUsageRecordCommand
     */     
	@PutMapping("/assignConnectivityPlan")
	public void assignConnectivityPlan( @RequestBody AssignConnectivityPlanToUsageRecordCommand command ) {
		try {
			service.assignConnectivityPlan( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign ConnectivityPlan", exc );
        }
	}

    /**
     * unassign ConnectivityPlan on UsageRecord
     * @param		 command UnAssignConnectivityPlanFromUsageRecordCommand
     */     
	@PutMapping("/unAssignConnectivityPlan")
	public void unAssignConnectivityPlan( @RequestBody(required=true)  UnAssignConnectivityPlanFromUsageRecordCommand command ) {
		try {
			service.unAssignConnectivityPlan( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign ConnectivityPlan", exc );
		}
	}
	


//************************************************************************    
// Attributes
//************************************************************************
    protected UsageRecord usageRecord = null;
	protected UsageRecordService service = null;
    private static final Logger LOGGER = Logger.getLogger(UsageRecordCommandRestController.class.getName());
    
}
