/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * IoT Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>DataRetentionPolicyCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity DataRetentionPolicy.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>IoT Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core IoT device, connectivity, telemetry, twin, command, security, and operations capabilities.</td></tr>
 *          <tr><td>industry</td><td>Internet of Things</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	DataRetentionPolicyService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateDataRetentionPolicyCommand</li>
 *      <li>UpdateDataRetentionPolicyCommand</li>
 *      <liDeleteDataRetentionPolicyCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/DataRetentionPolicyCommand")
public class DataRetentionPolicyCommandRestController extends BaseSpringRestController {

	public DataRetentionPolicyCommandRestController( DataRetentionPolicyService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a DataRetentionPolicy.
     * @param		DataRetentionPolicy	dataRetentionPolicy
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateDataRetentionPolicyCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createDataRetentionPolicy( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a DataRetentionPolicy.  if no key provided, calls create, otherwise calls save
     * @param		DataRetentionPolicy dataRetentionPolicy
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateDataRetentionPolicyCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateDataRetentionPolicyCommand
			// -----------------------------------------------
			completableFuture = service.updateDataRetentionPolicy(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "DataRetentionPolicyController:update() - successfully update DataRetentionPolicy - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a DataRetentionPolicy entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID dataRetentionPolicyId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteDataRetentionPolicyCommand command = new DeleteDataRetentionPolicyCommand( dataRetentionPolicyId );

    	try {
        	DataRetentionPolicyService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted DataRetentionPolicy with key " + command.getDataRetentionPolicyId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Tenant on DataRetentionPolicy
     * @param		command AssignTenantToDataRetentionPolicyCommand
     */     
	@PutMapping("/assignTenant")
	public void assignTenant( @RequestBody AssignTenantToDataRetentionPolicyCommand command ) {
		try {
			service.assignTenant( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Tenant", exc );
        }
	}

    /**
     * unassign Tenant on DataRetentionPolicy
     * @param		 command UnAssignTenantFromDataRetentionPolicyCommand
     */     
	@PutMapping("/unAssignTenant")
	public void unAssignTenant( @RequestBody(required=true)  UnAssignTenantFromDataRetentionPolicyCommand command ) {
		try {
			service.unAssignTenant( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Tenant", exc );
		}
	}
	

    /**
     * save Streams on DataRetentionPolicy
     * @param		command AssignStreamsToDataRetentionPolicyCommand
     */     
	@PutMapping("/addToStreams")
	public void addToStreams( @RequestBody(required=true) AssignStreamsToDataRetentionPolicyCommand command ) {
		try {
			service.addToStreams( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Streams", exc );
		}
	}

    /**
     * remove Streams on DataRetentionPolicy
     * @param		command RemoveStreamsFromDataRetentionPolicyCommand
     */     	
	@PutMapping("/removeFromStreams")
	public void removeFromStreams( 	@RequestBody(required=true) RemoveStreamsFromDataRetentionPolicyCommand command )
	{		
		try {
			service.removeFromStreams( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Streams", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected DataRetentionPolicy dataRetentionPolicy = null;
	protected DataRetentionPolicyService service = null;
    private static final Logger LOGGER = Logger.getLogger(DataRetentionPolicyCommandRestController.class.getName());
    
}
