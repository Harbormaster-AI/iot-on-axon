/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * IoT Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>ActuatorInstanceCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity ActuatorInstance.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>IoT Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core IoT device, connectivity, telemetry, twin, command, security, and operations capabilities.</td></tr>
 *          <tr><td>industry</td><td>Internet of Things</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	ActuatorInstanceService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateActuatorInstanceCommand</li>
 *      <li>UpdateActuatorInstanceCommand</li>
 *      <liDeleteActuatorInstanceCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/ActuatorInstanceCommand")
public class ActuatorInstanceCommandRestController extends BaseSpringRestController {

	public ActuatorInstanceCommandRestController( ActuatorInstanceService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a ActuatorInstance.
     * @param		ActuatorInstance	actuatorInstance
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateActuatorInstanceCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createActuatorInstance( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a ActuatorInstance.  if no key provided, calls create, otherwise calls save
     * @param		ActuatorInstance actuatorInstance
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateActuatorInstanceCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateActuatorInstanceCommand
			// -----------------------------------------------
			completableFuture = service.updateActuatorInstance(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "ActuatorInstanceController:update() - successfully update ActuatorInstance - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a ActuatorInstance entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID actuatorInstanceId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteActuatorInstanceCommand command = new DeleteActuatorInstanceCommand( actuatorInstanceId );

    	try {
        	ActuatorInstanceService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted ActuatorInstance with key " + command.getActuatorInstanceId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Device on ActuatorInstance
     * @param		command AssignDeviceToActuatorInstanceCommand
     */     
	@PutMapping("/assignDevice")
	public void assignDevice( @RequestBody AssignDeviceToActuatorInstanceCommand command ) {
		try {
			service.assignDevice( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Device", exc );
        }
	}

    /**
     * unassign Device on ActuatorInstance
     * @param		 command UnAssignDeviceFromActuatorInstanceCommand
     */     
	@PutMapping("/unAssignDevice")
	public void unAssignDevice( @RequestBody(required=true)  UnAssignDeviceFromActuatorInstanceCommand command ) {
		try {
			service.unAssignDevice( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Device", exc );
		}
	}
	

    /**
     * save SupportedCommands on ActuatorInstance
     * @param		command AssignSupportedCommandsToActuatorInstanceCommand
     */     
	@PutMapping("/addToSupportedCommands")
	public void addToSupportedCommands( @RequestBody(required=true) AssignSupportedCommandsToActuatorInstanceCommand command ) {
		try {
			service.addToSupportedCommands( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set SupportedCommands", exc );
		}
	}

    /**
     * remove SupportedCommands on ActuatorInstance
     * @param		command RemoveSupportedCommandsFromActuatorInstanceCommand
     */     	
	@PutMapping("/removeFromSupportedCommands")
	public void removeFromSupportedCommands( 	@RequestBody(required=true) RemoveSupportedCommandsFromActuatorInstanceCommand command )
	{		
		try {
			service.removeFromSupportedCommands( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set SupportedCommands", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected ActuatorInstance actuatorInstance = null;
	protected ActuatorInstanceService service = null;
    private static final Logger LOGGER = Logger.getLogger(ActuatorInstanceCommandRestController.class.getName());
    
}
