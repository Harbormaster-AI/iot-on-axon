/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * IoT Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>TenantUserCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity TenantUser.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>IoT Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core IoT device, connectivity, telemetry, twin, command, security, and operations capabilities.</td></tr>
 *          <tr><td>industry</td><td>Internet of Things</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	TenantUserService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateTenantUserCommand</li>
 *      <li>UpdateTenantUserCommand</li>
 *      <liDeleteTenantUserCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/TenantUserCommand")
public class TenantUserCommandRestController extends BaseSpringRestController {

	public TenantUserCommandRestController( TenantUserService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a TenantUser.
     * @param		TenantUser	tenantUser
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateTenantUserCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createTenantUser( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a TenantUser.  if no key provided, calls create, otherwise calls save
     * @param		TenantUser tenantUser
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateTenantUserCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateTenantUserCommand
			// -----------------------------------------------
			completableFuture = service.updateTenantUser(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "TenantUserController:update() - successfully update TenantUser - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a TenantUser entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID tenantUserId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteTenantUserCommand command = new DeleteTenantUserCommand( tenantUserId );

    	try {
        	TenantUserService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted TenantUser with key " + command.getTenantUserId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Tenant on TenantUser
     * @param		command AssignTenantToTenantUserCommand
     */     
	@PutMapping("/assignTenant")
	public void assignTenant( @RequestBody AssignTenantToTenantUserCommand command ) {
		try {
			service.assignTenant( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Tenant", exc );
        }
	}

    /**
     * unassign Tenant on TenantUser
     * @param		 command UnAssignTenantFromTenantUserCommand
     */     
	@PutMapping("/unAssignTenant")
	public void unAssignTenant( @RequestBody(required=true)  UnAssignTenantFromTenantUserCommand command ) {
		try {
			service.unAssignTenant( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Tenant", exc );
		}
	}
	

    /**
     * save CommandInvocations on TenantUser
     * @param		command AssignCommandInvocationsToTenantUserCommand
     */     
	@PutMapping("/addToCommandInvocations")
	public void addToCommandInvocations( @RequestBody(required=true) AssignCommandInvocationsToTenantUserCommand command ) {
		try {
			service.addToCommandInvocations( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set CommandInvocations", exc );
		}
	}

    /**
     * remove CommandInvocations on TenantUser
     * @param		command RemoveCommandInvocationsFromTenantUserCommand
     */     	
	@PutMapping("/removeFromCommandInvocations")
	public void removeFromCommandInvocations( 	@RequestBody(required=true) RemoveCommandInvocationsFromTenantUserCommand command )
	{		
		try {
			service.removeFromCommandInvocations( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set CommandInvocations", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected TenantUser tenantUser = null;
	protected TenantUserService service = null;
    private static final Logger LOGGER = Logger.getLogger(TenantUserCommandRestController.class.getName());
    
}
