/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * IoT Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>ConnectivityPlanCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity ConnectivityPlan.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>IoT Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core IoT device, connectivity, telemetry, twin, command, security, and operations capabilities.</td></tr>
 *          <tr><td>industry</td><td>Internet of Things</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	ConnectivityPlanService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateConnectivityPlanCommand</li>
 *      <li>UpdateConnectivityPlanCommand</li>
 *      <liDeleteConnectivityPlanCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/ConnectivityPlanCommand")
public class ConnectivityPlanCommandRestController extends BaseSpringRestController {

	public ConnectivityPlanCommandRestController( ConnectivityPlanService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a ConnectivityPlan.
     * @param		ConnectivityPlan	connectivityPlan
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateConnectivityPlanCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createConnectivityPlan( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a ConnectivityPlan.  if no key provided, calls create, otherwise calls save
     * @param		ConnectivityPlan connectivityPlan
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateConnectivityPlanCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateConnectivityPlanCommand
			// -----------------------------------------------
			completableFuture = service.updateConnectivityPlan(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "ConnectivityPlanController:update() - successfully update ConnectivityPlan - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a ConnectivityPlan entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID connectivityPlanId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteConnectivityPlanCommand command = new DeleteConnectivityPlanCommand( connectivityPlanId );

    	try {
        	ConnectivityPlanService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted ConnectivityPlan with key " + command.getConnectivityPlanId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Tenant on ConnectivityPlan
     * @param		command AssignTenantToConnectivityPlanCommand
     */     
	@PutMapping("/assignTenant")
	public void assignTenant( @RequestBody AssignTenantToConnectivityPlanCommand command ) {
		try {
			service.assignTenant( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Tenant", exc );
        }
	}

    /**
     * unassign Tenant on ConnectivityPlan
     * @param		 command UnAssignTenantFromConnectivityPlanCommand
     */     
	@PutMapping("/unAssignTenant")
	public void unAssignTenant( @RequestBody(required=true)  UnAssignTenantFromConnectivityPlanCommand command ) {
		try {
			service.unAssignTenant( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Tenant", exc );
		}
	}
	

    /**
     * save SimCards on ConnectivityPlan
     * @param		command AssignSimCardsToConnectivityPlanCommand
     */     
	@PutMapping("/addToSimCards")
	public void addToSimCards( @RequestBody(required=true) AssignSimCardsToConnectivityPlanCommand command ) {
		try {
			service.addToSimCards( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set SimCards", exc );
		}
	}

    /**
     * remove SimCards on ConnectivityPlan
     * @param		command RemoveSimCardsFromConnectivityPlanCommand
     */     	
	@PutMapping("/removeFromSimCards")
	public void removeFromSimCards( 	@RequestBody(required=true) RemoveSimCardsFromConnectivityPlanCommand command )
	{		
		try {
			service.removeFromSimCards( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set SimCards", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected ConnectivityPlan connectivityPlan = null;
	protected ConnectivityPlanService service = null;
    private static final Logger LOGGER = Logger.getLogger(ConnectivityPlanCommandRestController.class.getName());
    
}
